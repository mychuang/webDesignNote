...
async function queryCameraConfig() {
  ...
  camera._config.CV605 = Number(await camera.getCV605())
  if(camera._config.CV605 == 0){
...
function updateUI(setEnable: boolean) {
  ...
  if (setEnable) {
    ...
    if(camera._config.CV605 == 0){
      $("#init_position").html(
        "<option value='2'>1st Preset</option><option value='3'>Last MEM</option>"
      );
      (<HTMLSelectElement>document.getElementById("init_position")).value = String(camera._config.init_position);
    }else if(camera._config.CV605 == 1){
      $("#init_position").html(
        "<option value='99'></option><option value='4'>Last MEM Enable</option><option value='5'>Last MEM Disable</option>"
      );
      (<HTMLSelectElement>document.getElementById("init_position")).value = String(camera._config.init_position);
    }
...
document.getElementById('init_position')?.addEventListener("change", (event) => {
  ...
  if (initPosition) {
    ...
    if(camera._config.CV605 == 0){
      ...
      } else if(camera._config.init_position === InitPosition.LastMem){
        camera.sendCommand(LumensCommand.init_position_last_mem)
      }
    }else if(camera._config.CV605 == 1){
      if (camera._config.init_position === InitPosition.LastMemOn) {
        camera.sendCommand(LumensCommand.init_position_last_mem_on)
      } else if ((camera._config.init_position === InitPosition.LastMemOff)) {
        camera.sendCommand(LumensCommand.init_position_1ast_mem_off)
      }
    }
  }
})

...
function queryCameraConfig() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        index_1.camera._config.CV605 = Number(yield index_1.camera.getCV605());
        if (index_1.camera._config.CV605 == 0) {
...
function updateUI(setEnable) {
    ...
    if (setEnable) {
        ...
        if (index_1.camera._config.CV605 == 0) {
            $("#init_position").html("<option value='2'>1st Preset</option><option value='3'>Last MEM</option>");
            document.getElementById("init_position").value = String(index_1.camera._config.init_position);
        }
        else if (index_1.camera._config.CV605 == 1) {
            $("#init_position").html("<option value='99'></option><option value='4'>Last MEM Enable</option><option value='5'>Last MEM Disable</option>");
            document.getElementById("init_position").value = String(index_1.camera._config.init_position);
        }
    }
    else {
...
(_b = document.getElementById('init_position')) === null || _b === void 0 ? void 0 : _b.addEventListener("change", (event) => {
    ...
    if (initPosition) {
        ...
        if (index_1.camera._config.CV605 == 0) {
            ...
            else if (index_1.camera._config.init_position === Camera_1.InitPosition.LastMem) {
                index_1.camera.sendCommand(Camera_1.LumensCommand.init_position_last_mem);
            }
        }
        else if (index_1.camera._config.CV605 == 1) {
            if (index_1.camera._config.init_position === Camera_1.InitPosition.LastMemOn) {
                index_1.camera.sendCommand(Camera_1.LumensCommand.init_position_last_mem_on);
            }
            else if ((index_1.camera._config.init_position === Camera_1.InitPosition.LastMemOff)) {
                index_1.camera.sendCommand(Camera_1.LumensCommand.init_position_1ast_mem_off);
            }
        }
    }
});
